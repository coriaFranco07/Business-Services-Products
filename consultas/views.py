...
def contacto(request):
    if request.method == "POST":
        ...
        if formulario_contacto.is_valid(): # Verificamos si los datos son validos
            ...
            try:
                email.send()
                return redirect("/consultas/?valido") # Redireccionamos de nuevo a la pagina contactos si es valido, pero con el parametro de los datos enviados
            except:
                return redirect("/consultas/?novalido")
    else:
        formulario_contacto = formularioContacto()  # Crear formulario vacío para GET


        return render(request, "consultas/consultas.html", {'miFormulario':formulario_contacto})
